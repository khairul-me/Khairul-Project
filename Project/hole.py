...
class FocusedScanner:
    ...
    def add_reference_lines(self, ax):
        ...
        # Center line
        ax.plot([0, 0], [0, 75], '--', color='gray', alpha=0.3)
    ...
    def detect_gap(self, distance):
        """Detect gaps based on distance or lack of echo."""
        if distance is None:  # No echo detected
            return True
        if distance > self.min_distance + 3:  # Threshold for detecting a gap
            return True
        return False
    ...
    def read_sensor_data(self):
        ...
        try:
            ...
            if "Angle" in line and "Distance" in line:
                ...
                # Check for gaps
                is_gap = self.detect_gap(distance)

                # Only process data within valid range or if it's a gap
                if distance >= self.min_distance or is_gap:
                    x, y = self.process_point(angle, distance if not is_gap else None)
                    data_point = {
    ...
    def update_plot(self, frame):
        ...
        points = [self.process_point(a, d) for a, d in zip(self.angle_buffer, self.distance_buffer)]
        x_points = [p[0] for p in points]
        y_points = [p[1] for p in points]

        # Plot 1: Real-time scan data
        scatter = self.ax1.scatter(x_points, y_points,
                                    c=['red' if self.detect_gap(d) else 'green' for d in self.distance_buffer],
                                    cmap='viridis',
                                    s=30,
                                    alpha=0.6)

        # Add current scan line
        current_x, current_y = self.process_point(angle, 75)
        ...
        try:
            with open(filename, 'w', newline='') as file:
                fieldnames = ['timestamp', 'angle', 'distance', 'x', 'y', 
                            'state', 'direction', 'time_elapsed', 'scan_cycle', 'is_gap']
