...
class EnhancedGapDetector:
    ...
    def detect_gap(self, distance, angle):
        """Enhanced gap detection with absolute difference threshold"""
        if distance is None:
            return False
        ...
        if self.baseline_distance:
            distance_diff = abs(smoothed_distance - self.baseline_distance)
            is_gap = distance_diff > self.gap_threshold
